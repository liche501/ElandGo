...
func (this *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if len(idstr) > 0 {
			userObj := this.GetSession("user_obj")
			if userObj == nil {
				user, err := models.UserGetById(idstr)
				if err == nil && password == libs.Md5([]byte(this.getClientIp()+"|"+user.Password)) {
					this.userId = user.UserId
					this.userName = user.UserName
					this.user = user
					this.SetSession("user_obj", user)
				}
			} else {
				//				this.userId = userObj.(BaseController).userId
				//				beego.Alert(userObj.(*models.User).UserId)
				see_user := userObj.(*models.User)
				this.userId = see_user.UserId
				this.userName = see_user.UserName
				this.user = see_user
			}
		}
	}
	if this.userId == "" && (this.controllerName != "main" ||
